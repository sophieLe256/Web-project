...
export const Accessories = () => {
  ...
  const accessoriesData = DUMMY_DATA.filter((product) => product.categories === "Accessories");

  const startIndex = (currentPage - 1) * productsPerPage;
  const endIndex = startIndex + productsPerPage;
  const paginatedData = accessoriesData.slice(startIndex, endIndex);

  const totalPages = Math.ceil(accessoriesData.length / productsPerPage);
