...
export const Tshirts = () => {
  ...
  const tshirtData = DUMMY_DATA.filter((product) => product.categories === "T-Shirts");

  const startIndex = (currentPage - 1) * productsPerPage;
  const endIndex = startIndex + productsPerPage;
  const paginatedData = tshirtData.slice(startIndex, endIndex);

  const totalPages = Math.ceil(tshirtData.length / productsPerPage);
