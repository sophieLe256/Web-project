...
export const Jackets = () => {
  ...
  const jacketsData = DUMMY_DATA.filter((product) => product.categories === "Jackets");

  const startIndex = (currentPage - 1) * productsPerPage;
  const endIndex = startIndex + productsPerPage;
  const paginatedData = jacketsData.slice(startIndex, endIndex);

  const totalPages = Math.ceil(jacketsData.length / productsPerPage);
