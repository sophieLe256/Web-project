        ...
        <div class="product-detail">
          <div class="container-fluid p-0">
            <div class="product-image-sliders">
                ...
                <div class="swiper-container swiper-styled swiper-container-initialized swiper-container-horizontal swiper-container-pointer-events">
                  ...
                  >
                    ...
                    >
                      <img
                        ...
                        class=" lazyloaded"
                        data-src="//product.hstatic.net/1000351433/product/ms_669dea3d9dc94b958983b70b698649e9_master.jpg"
                        alt=" SPECIAL RABBIT TEE "
                      />
                  ...
                  >
                    ...
                    <img
                      ...
                      class=" lazyloaded"
                      data-src="//product.hstatic.net/1000351433/product/rabbit-special-tee_ebe7ea140fc248c49c78e76bcd6cd1e1_master.jpg"
                      alt=" SPECIAL RABBIT TEE "
                    />
              ...
              <div
                ...
                aria-controls="swiper-wrapper-51e28659bd18c727"
              ></div>
              <span
          ...
          <div class="product-info">
            ...
            <div class="product-price-wrap">
              <span class="product-price font-weight-bold">490.000 VND </span>
            </div>
            ...
            <div class="product-options product-options-1050687980">
              <div class="option option-size option-1 d-flex align-items-center">
                <span class="text-uppercase font-weight-bold">size:</span>
...
<script>
	...
	!function(){
		...
		var option_size = $('.product-options-1050687980.product-options .option').length;

		for(var variant_title in variants) {
			variants[variant_title].option1 && document.querySelector('.product-options-1050687980 .option-1 .option-values [data-option-value="' + variants[variant_title].option1 + '"]').classList.remove('disabled');
			variants[variant_title].option2 && document.querySelector('.product-options-1050687980 .option-2 .option-values [data-option-value="' + variants[variant_title].option2 + '"]').classList.remove('disabled');
			variants[variant_title].option3 && document.querySelector('.product-options-1050687980 .option-3 .option-values [data-option-value="' + variants[variant_title].option3 + '"]').classList.remove('disabled');
		};

		$(document).on('click', '.product-options-1050687980 .option .option-values > span:not(.disabled)', function(){
			var $selected_option = $(this).addClass('active');
			var option_value = $selected_option.attr('data-option-value');
			var option_parent_idx = $selected_option.closest('.option').index() + 1;


			$selected_option.siblings().removeClass('active');

			// mỗi lần chọn option, quét lại để bỏ option ko khả dụng
			if(option_parent_idx === 1){
				$('.product-options-1050687980 .option-2 .option-values > span').addClass('disabled');

				for(var variant_title in variants) {
					if(variants[variant_title].option1 === option_value) {
						variants[variant_title].option2 && document.querySelector('.product-options-1050687980 .option-2 .option-values [data-option-value="' + variants[variant_title].option2 + '"]').classList.remove('disabled');
					}
				}

				// auto chọn option 2,3 or để nguyên như cũ
				$('.product-options-1050687980 .option-2 .option-values > span.active.disabled').length && $('.product-options-1050687980 .option-2 .option-values > span.active.disabled').removeClass('active');

				if($('.product-options-1050687980 .option-2 .option-values > span.active').length) {
					$('.product-options-1050687980 .option-2 .option-values > span.active').trigger('click');
				} else {
					$('.product-options-1050687980 .option-2 .option-values > span:not(.disabled)').eq(0).trigger('click')
				}

			}
			if(option_parent_idx === 2){
				$('.product-options-1050687980 .option-3 .option-values > span').addClass('disabled');

				for(var variant_title in variants) {
					if(variants[variant_title].option2 === option_value && variants[variant_title].option1 === $('.product-options-1050687980 .option-1 .option-values > span.active').attr('data-option-value')) {
						variants[variant_title].option3 && $('.product-options-1050687980 .option-3 .option-values [data-option-value="' + variants[variant_title].option3 + '"]').removeClass('disabled');
					}
				}

				// auto chọn option 2,3 or để nguyên như cũ
				$('.product-options-1050687980 .option-3 .option-values > span.active.disabled').length && $('.product-options-1050687980 .option-3 .option-values > span.active.disabled').removeClass('active');


				if($('.product-options-1050687980 .option-3 .option-values > span.active').length) {
					$('.product-options-1050687980 .option-3 .option-values > span.active').trigger('click');
				} else {
					$('.product-options-1050687980 .option-3 .option-values > span:not(.disabled)').eq(0).trigger('click')
				}

			}
			if(option_parent_idx === option_size) {
				$('#selected-variant-1050687980').val(variants[$('.product-options-1050687980.product-options .option .option-values span.active').map(function(idx, elm){
					return $(elm).text().trim();
				}).get().join(' / ')].id);
        var html_price = '';
        var price = variants[$('.product-options-1050687980.product-options .option .option-values span.active').map(function(idx, elm){
					return $(elm).text().trim();
				}).get().join(' / ')].price;
        var compare_at_price = variants[$('.product-options-1050687980.product-options .option .option-values span.active').map(function(idx, elm){
					return $(elm).text().trim();
				}).get().join(' / ')].compare_at_price;
        if(compare_at_price > price){
          html_price += '<del>'+Haravan.formatMoney(compare_at_price,shop.formatMoney)+'</del>';
        }
        html_price += '<span class="product-price font-weight-bold">'+Haravan.formatMoney(price,shop.formatMoney)+'</span>';
        $('.product-detail .product-info .product-price-wrap').html(html_price);
			}

		});
		$('.product-options-1050687980 .option-1 .option-values > span:not(.disabled)').eq(0).trigger('click');
	}();
  })
</script> */}
              ...
              <div class="bluecore-reviews-head">
                ...
                <div class="bluecore-histogram reviews-col-50">
                  <div class="bluecore-histogram-content">
                    <h4 class="bluecore-histogram-title">
                      Product available in the right size?
                    </h4>
                    <div class="bluecore-histogram-block">
